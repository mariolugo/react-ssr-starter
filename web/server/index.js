...
/*
 * NB: make sure to modify this to take into account anything that should
...
app.prepare().then(() => {
  ...
  server.get('/manifest.html', (req, res) => {
    return app.serveStatic(req, res, path.resolve('./.next/manifest.html'));
  });

  server.get('/manifest.appcache', (req, res) => {
    return app.serveStatic(req, res, path.resolve('./.next/manifest.appcache'));
  });

  if (isProd) {
    server.get('/_next/-/app.js', (req, res) => {
      return app.serveStatic(req, res, path.resolve('./.next/app.js'));
    });
